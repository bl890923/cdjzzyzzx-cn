...
$(function() {
   // $("").unbind().bind("click", searchKeyWord);
    ...
    //搜索框   开始
    $(".icon-search btnSearch").unbind().bind("click", function () {
        var url = "";
        var type = "product";
        var kwd = $("#se_key").val();
...
function submitOrder(UserName, Phone, Email, OrderInfo, _oid) {
    ...
    if ($("#" + UserName + "").val() == "" || $("#" + UserName + "").val() == "请输入姓名") {
        ...
        return;
    } else if ($("#" + Email + "").val() == "" || $("#" + Email + "").val() == "请输入邮箱") {
        $("#ErrorInfo").empty();
        $("#ErrorInfo").append("请输入邮箱");
        return;
