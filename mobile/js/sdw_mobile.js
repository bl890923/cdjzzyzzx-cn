...
function sendLeaveword() {
    ...
    if ($("#LEAVEWORD_txtContact").val() == "" || $("#LEAVEWORD_txtContact").val() == "姓名") {
        $("#MsgInfos").empty();
        $("#MsgInfos").append("姓名不能为空");
        return;
    } else if ($("#LEAVEWORD_txtMobile").val() == "" || $("#LEAVEWORD_txtMobile").val() == "手机") {
        $("#MsgInfos").empty();
        $("#MsgInfos").append("手机不能为空");
        return;
    } else if (!zz.test($("#LEAVEWORD_txtMobile").val())) {
        $("#MsgInfos").empty();
        $("#MsgInfos").append("手机号码格式不正确");
        return;
    } else if ($("#LEAVEWORD_txtShortDesc").val() == "" || $("#LEAVEWORD_txtShortDesc").val() == "留言") {
        $("#MsgInfos").empty();
        $("#MsgInfos").append("请输入留言信息");
        return;
    } else {
        $("#MsgInfos").empty();
        $.post("/ajax.ashx?action=MobileLeaveword&t=" + Math.random(), {
            desc: $("#LEAVEWORD_txtShortDesc").val(),
            ...
            if (sta == "1") {
                ...
                $("#MsgInfos").show(1000, function () {
                    $("#LEAVEWORD_txtShortDesc").val("留言");
                    $("#LEAVEWORD_txtContact").val("姓名");
                    $("#LEAVEWORD_txtMobile").val("手机");
                });
            } else {
